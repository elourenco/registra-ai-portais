...
import { formatCnpj as formatSupplierCnpj } from "@/features/operations/core/operations-presenters";
...
export function DevelopmentRegistrationPage() {
  ...
  const handleSubmit = form.handleSubmit((values) => {
    setSavedSummary(
      `${values.name} vinculado à incorporadora ${
        selectedSupplier?.name ?? values.supplierCustomName ?? "selecionada"
      }. Empreendimento validado e pronto para persistência.`,
    );
  });
        ...
        <Card className="border-emerald-200 bg-emerald-50/80">
          <CardContent className="p-5">
            ...
            <p className="mt-1 text-sm text-emerald-700/80">
              Validação concluída no frontend. A persistência ainda será conectada à API operacional.
            </p>
          </CardContent>
        </Card>
      ) : null}
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {workspaceQuery.data?.suppliers.map((supplier) => (
